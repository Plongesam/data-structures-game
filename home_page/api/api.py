...
def rankings(request, top_n):
    ...
    for ii, cursor in enumerate(db.get_rankings()):
        ...
        temp = {'user_id': cursor['user_id'], 'points': round(cursor['points'], 2, 'llistStandardPoints': round(cursor['llistStandardPoints'], 2, 'llistSurvivalPoints': round(cursor['llistSurivalPoints'], 2)))}
